...
Runs the stages in order as subprocesses. Run it from the REPO ROOT so the
data dirs (runs/ bench/ docs/) resolve relative to the working directory:

    .venv/bin/python submission/run_pipeline.py                 # compose -> dashboard (cheap; uses committed bench/*)
    .venv/bin/python submission/run_pipeline.py --from gatetracker
    .venv/bin/python submission/run_pipeline.py --only compose,systemsmap
    .venv/bin/python submission/run_pipeline.py --topics --explore   # full rebuild incl. web-grounded stages
    .venv/bin/python submission/run_pipeline.py --dry-run
...
Stage map (● = web-grounded / expensive, needs the Claude Code login):
    ...
    2 prototype     per-topic decision graph            -> runs/<t>/decisiongraph.json ● (needs runs/)
...
Stages 1-2 need runs/ (gitignored). From a fresh clone start at `compose`
...
ROOT = os.getcwd()                                          # repo root (run from here)
...
def build_steps(topics: bool, explore: bool) -> list[tuple[str, list[list[str]]]]:
    ...
    steps += [
        ("prototype", [_script("run_prototype.py", t) for t in tids]),
