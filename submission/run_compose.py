...
Prereq: run the decision-graph stage on each topic first, e.g.
    for t in t1 t2 t3 t4 t5 t7 t8 t9; do python run_prototype.py $t; done
...
def main() -> int:
    ...
    if not graphs:
        print("no decisiongraph.json files found — run run_prototype.py on the topics first."); return 1
