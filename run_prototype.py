#!/usr/bin/env python3
"""PROTOTYPE — decision-graph crux stage (replaces the binary-yes/no crux step).

Runs the new synthesis on ONE topic's EXISTING frozen notes (no re-explore), so we
can compare the decision-graph output head-to-head with the old 4-6 binary cruxes.

    python run_prototype.py t6

Reads runs/<id>-*/notes.merged.json (or notes.json), calls the deep model once with
the Variable + Edge + ForecastSignal schema, and writes:
...
from forecaster.pipeline import _format_graph
